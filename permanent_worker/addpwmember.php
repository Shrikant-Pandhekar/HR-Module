...
if (strlen($_SESSION['login']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $staff_id = $letters . substr(str_shuffle($numbers), 0, 3);
            ...
            $query = mysqli_query($con, "insert into p_worker (pw_id, firstname, lastname, address, dob, gender, email, phone, position, sal, photo) values ('$staff_id', '$firstname', '$lastname', '$address', '$dob', '$gender', '$email' , '$phone' , '$position', '$sal', '$imgnewfile')");

            mysqli_query($con, $query);
            $last = mysqli_insert_id($con);
 
            $sid = mysqli_query($con, "SELECT pw_id FROM p_worker where id='$last'");
  
            while ($row = mysqli_fetch_array($sid)) {
                echo '<script> alert("Permanent Worker successfully inserted with pw id : "+"' . $row['pw_id'] . '")</script>';
            }
...
?>
    ...
    <head>
        ...
        <title>HR Module | Add Permanent Workers</title>
    ...
    <body>
        <div class="wrapper">
            ...
            <div id="content" style="width:100%">
                ...
                <h1 class="h2 mx-3">Add Permanent Workers</h1><br>
                <div class="container">
