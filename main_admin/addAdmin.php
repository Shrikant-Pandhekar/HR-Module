...
if (strlen($_SESSION['login']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $admin_id = $letters . substr(str_shuffle($numbers), 0, 3);
        ...
        $subject = "HR Module Log In";
        $body = "Password For log In: " . $pass . "\n";
        $body .= "<b>*DO NOT SHARE WITH ANYONE*</b>";
        $headers = "From: HR MODULE";
        ...
        $query = mysqli_query($con, "insert into admin (admin_id, firstname, lastname, address, dob, gender, email, password, setPass , phone, position, sal, photo) values ('$admin_id', '$firstname', '$lastname', '$address', '$dob', '$gender', '$email' , '$pass','$set' ,'$phone' , '$position', '$sal', '$imgnewfile')");

        mysqli_query($con, $query);
        $last = mysqli_insert_id($con);
        $sid = mysqli_query($con, "SELECT admin_id FROM admin where id='$last'");
        while ($row = mysqli_fetch_array($sid)) {
            echo '<script> alert("Sub Admin successfully inserted with id : "+"' . $row['admin_id'] . '")</script>';
        }
    }
?>
    <!DOCTYPE html>
                                                ...
                                                <div class="row">
                                                    <div class="col-md-6">
                                                        <div class="form-group">
                                                            ...
                                                            <select class="form-control" name="position" id="form_name" required>
                                                                <option value="none" selected disabled hidden>
                                                                    Select an Position
                                                                </option>
                                                                <option value="staff">Staff Admin</option>
                                                                <option value="p_worker">Permanent Worker Admin</option>
                                                                <option value="c_worker">Casual Worker Admin</option>
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
